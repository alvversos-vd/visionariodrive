...
export default function SessionOverlays({ refresh }: { refresh: number }) {
  ...
  const { goals, snapshot } = useDashboard(refresh);
  const yesterday = useDayMetrics(YESTERDAY);

  if (phase === 'welcome') {
    return (
      <SessionWelcome
        metaDaily={goals.daily}
        lucroPorHora={snapshot.today.profitPerHour || yesterday.profitPerHour}
